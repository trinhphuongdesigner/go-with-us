...
/**
 * M4 — the owner's view of their portable career record: employment
 * periods, the AI recap of each one, and the share links that let a new
 * employer read it.
 */
export default function CareerPassportPage() {
  ...
  const load = React.useCallback(async () => {
    ...
    setError(null);
    try {
      const [data, shareList] = await Promise.all([
        getCareerPassport(),
        listPassportShares(),
      ]);
      setPassport(data);
    ...
    <AppShell>
      <PageContainer>
        <PageHeader
          ...
          subtitle="Your record across employers — it stays yours even after you leave a company."
          actions={
            <Button
              variant="outlined"
              startIcon={<AddIcon />}
