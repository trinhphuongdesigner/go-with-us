import type { CoreProfile, PeopleList, PersonDetail, ProfileUpdateRequest } from "@/lib/api";
...
export function listDemoPeople(session: Session): PeopleList {
  if (!session.user.companyId) return { items: [], total: 0 };
  const profiles = [profileFor(employeeProfile.id), profileFor(engineerProfile.id), profileFor(adminProfile.id)];
  return {
    items: profiles.map((profile, index) => ({
      id: profile.id,
      name: profile.name,
      jobTitle: profile.jobTitle,
      department: index === 0 ? "Sản phẩm" : index === 1 ? "Kỹ thuật" : "Nhân sự",
...
export function getDemoPerson(session: Session, employeeId: string): PersonDetail {
  const person = listDemoPeople(session).items.find((candidate) => candidate.id === employeeId);
  ...
  return { ...profileFor(employeeId), department: person.department };
}
