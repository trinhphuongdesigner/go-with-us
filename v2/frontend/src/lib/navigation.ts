...
export function getCompanyNavigation(companyId: string, permissions: Permission[]): NavigationItem[] {
  ...
  const scoped: NavigationItem[] = [
    { label: "Không gian công ty", shortLabel: "Công ty", href: `/cong-ty-cua-toi${query}`, icon: Building2, permission: "dashboard:read", exact: true },
    { label: "Nhân sự", shortLabel: "Nhân sự", href: `/nhan-su${query}`, icon: UsersRound, permission: "people:read" as const },
    { label: "Bộ tiêu chí đánh giá", shortLabel: "Bộ tiêu chí", href: `/cong-ty/tieu-chi${query}`, icon: BadgeCheck, permission: "assessment:review" },
    { label: "Chu kỳ & lượt đánh giá", shortLabel: "Đánh giá", href: `/danh-gia${query}`, icon: BadgeCheck, permission: "assessment:self", anyPermissions: ["assessment:self", "assessment:review", "passport:approve"] },
