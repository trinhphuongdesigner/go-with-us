...
import { MembershipManager, PasswordReset } from "./company-memberships";
...
const grantLabels: Record<string, string> = { COMPANY_READ: "Xem công ty", COMPANY_WRITE: "Quản lý công ty", EMPLOYEE_READ: "Xem nhân sự", EMPLOYEE_WRITE: "Quản lý nhân sự", ASSESSMENT_REVIEW: "Duyệt đánh giá", PASSPORT_APPROVE: "Duyệt hộ chiếu", MANAGE_ROLES: "Quản lý phân quyền HR/BOD" };
...
export function OrganizationWorkspace({ initialCompanyId, initialPanel }: { initialCompanyId?: string; initialPanel?: "roles" }) {
  ...
  return <Workspace key={`${session.user.id}:${initialCompanyId ?? ""}:${initialPanel ?? ""}`} initialCompanyId={initialCompanyId} initialPanel={initialPanel} />;
}

function Workspace({ initialCompanyId, initialPanel }: { initialCompanyId?: string; initialPanel?: "roles" }) {
  const { session } = useAuth();
  ...
  const [tab, setTab] = useState<"company" | "users" | "roles">(initialPanel ?? "users");
  const [editing, setEditing] = useState<Account | "new" | null>(null);
  const [resetUserId, setResetUserId] = useState("");
  const [notice, setNotice] = useState("");
  const companies = useQuery<Company[]>({ queryKey: ["organization-companies", actor.id, initialPanel], queryFn: async () => initialPanel === "roles" ? (await listAvailableCompanies(session!)).map((item) => ({ ...item, industry: null, status: "ACTIVE" as const, version: 1 })) : apiRequest<Company[]>("/organization/companies", undefined, token), enabled: initialPanel === "roles" || actor.permissions.includes("company:read") });
  const company = selectedId ? companies.data?.find((item) => item.id === selectedId) : companies.data?.[0];
  const companyId = selectedId || company?.id || actor.companyId || "";
  const scope = `?companyId=${encodeURIComponent(companyId)}`;
  const users = useQuery({ queryKey: ["organization-users", actor.id, companyId], queryFn: () => apiRequest<Account[]>(`/organization/users${scope}`, undefined, token), enabled: initialPanel !== "roles" && !!companyId && actor.permissions.includes("people:read") });
  const roles = useQuery({ queryKey: ["organization-roles", actor.id, companyId], queryFn: () => apiRequest<Grants[]>(`/organization/roles${scope}`, undefined, token), enabled: !!companyId && canManageRoles });
  ...
  return <div className="space-y-6">
    {initialPanel !== "roles" && companyId ? <MembershipManager companyId={companyId} /> : null}
    {canWriteUsers && initialPanel !== "roles" ? <section className="space-y-3"><label className="block text-sm font-semibold">Khôi phục mật khẩu tài khoản<select className={fieldClass} value={resetUserId} onChange={(event) => setResetUserId(event.target.value)}><option value="">Chọn tài khoản cần đặt lại mật khẩu</option>{users.data?.filter((user) => user.id !== actor.id && (superAdmin || roleOrder.indexOf(user.role) > roleOrder.indexOf(actor.role))).map((user) => <option key={user.id} value={user.id}>{user.name} · {user.email}</option>)}</select></label>{users.data?.some((user) => user.id === resetUserId) ? <PasswordReset key={resetUserId} userId={resetUserId} /> : null}</section> : null}
    ...
    <div role="group" aria-label="Chức năng quản trị" className="flex flex-wrap gap-2">{([['users', 'Tài khoản'], ['company', 'Thông tin công ty'], ['roles', 'Cấp quyền']] as const).filter(([key]) => initialPanel === "roles" ? key === "roles" : key !== "roles" || canManageRoles).map(([key, label]) => <Button key={key} variant={tab === key ? "primary" : "secondary"} aria-pressed={tab === key} onClick={() => setTab(key)}>{label}</Button>)}</div>
