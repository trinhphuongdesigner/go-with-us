...
import type { RoadmapCategory } from "@/features/roadmap/roadmap-api";
import { careerRequest, type Goal, type GoalDraft, type Plan } from "./api";
import { ErrorNotice, fieldClass, panelClass } from "./settings";

const statusLabels = { NOT_STARTED: "Chưa bắt đầu", IN_PROGRESS: "Đang thực hiện", ACHIEVED: "Đã đạt" };
const emptyGoal = (category: RoadmapCategory): GoalDraft => ({ title: "", category, description: null, metric: null, targetValue: null, currentValue: null, progress: 0, dueDate: null, status: "NOT_STARTED", aiSuggested: false });

function isGoalVersionConflict(error: unknown) {
  if (!(error instanceof ApiError) || error.status !== 409 || !error.details || typeof error.details !== "object") return false;
  const detail = "detail" in error.details ? error.details.detail : null;
  return Boolean(detail && typeof detail === "object" && "code" in detail && detail.code === "version_conflict");
}

function GoalMutationNotice({ saveError, deleteError, refreshFailed }: { saveError: unknown; deleteError: unknown; refreshFailed: boolean }) {
  if (isGoalVersionConflict(saveError)) return <p role="alert" className="text-sm text-danger">{refreshFailed ? "Mục tiêu đã thay đổi ở phiên khác. Không tải được dữ liệu mới; bản nháp của bạn vẫn được giữ. Hãy thử tải lại trước khi lưu." : "Mục tiêu đã thay đổi ở phiên khác. Dữ liệu mới đã được tải lại; bản nháp của bạn vẫn được giữ."}</p>;
  if (isGoalVersionConflict(deleteError)) return <p role="alert" className="text-sm text-danger">{refreshFailed ? "Mục tiêu đã thay đổi ở phiên khác. Không tải được dữ liệu mới; yêu cầu xóa chưa được thực hiện. Hãy thử tải lại trước khi xóa." : "Mục tiêu đã thay đổi ở phiên khác. Dữ liệu mới đã được tải lại; yêu cầu xóa chưa được thực hiện."}</p>;
...
export function CareerPlanPanel({ session, category }: { session: Session; category: RoadmapCategory }) {
  const token = session.accessToken;
  const client = useQueryClient();
  const key = ["career-plan", session.user.id, category];
  const goals = useQuery({ queryKey: [...key, "goals"], queryFn: () => careerRequest<Goal[]>(token, `/development-plans/goals?category=${category}`) });
  const history = useQuery({ queryKey: [...key, "history"], queryFn: () => careerRequest<Plan[]>(token, `/development-plans/me/history?category=${category}`) });
  ...
  const goalSave = useMutation({ mutationFn: (value: GoalDraft) => {
    ...
    return careerRequest<Goal>(
      token,
      `/development-plans/goals${target ? `/${target.id}` : ""}`,
      target ? "PATCH" : "POST",
      target ? { ...value, expectedVersion: target.version } : value,
    );
