...
async def test_tenant_isolation(db_session: AsyncSession) -> None:
    ...
    user_a = await create_user(
        db_session, email="a@a.dev", password="Password123!", company=company_a
    )
    await create_user(db_session, email="b@b.dev", password="Password123!", company=company_b)
    await create_employment(
        db_session, user=user_a, company=company_a, title="Engineer", years_ago_start=2
    )
...
async def test_title_term_filters_candidates(db_session: AsyncSession) -> None:
    ...
    await create_employment(
        db_session, user=sales, company=company, title="Sales Rep", years_ago_start=3
    )
...
async def test_experience_years_computed_from_employment_dates(db_session: AsyncSession) -> None:
    ...
    await create_employment(
        db_session, user=user, company=company, title="Engineer", years_ago_start=4
    )
...
@pytest.mark.asyncio
async def test_requires_active_employment_and_filters_before_limit(
    db_session: AsyncSession,
) -> None:
    company = await create_company(db_session, "Eligible")
    ended = await create_user(
        db_session, email="a-ended@x.dev", password="Password123!", company=company
    )
    await create_employment(
        db_session,
        user=ended,
    ...
    for index in range(26):
        ...
        await create_employment(
            db_session, user=user, company=company, title="Sales", years_ago_start=2
        )
    ...
    await create_employment(
        db_session, user=match, company=company, title="Engineer", years_ago_start=2
    )

    rows = await search_candidates(
        db_session, company_id=company.id, title_terms=["Engineer"], limit=25
    )
...
@pytest.mark.asyncio
async def test_required_terms_are_and_preferred_terms_do_not_filter(
    db_session: AsyncSession,
) -> None:
    ...
    one = await create_user(
        ...
        password="Password123!",
        company=company,
        job_title="React Python",
    )
    for user in (both, one):
        await create_employment(
            db_session, user=user, company=company, title=user.job_title or "", years_ago_start=3
        )
...
async def test_minimum_total_experience_is_hard_filter(db_session: AsyncSession) -> None:
    ...
    junior = await create_user(
        db_session, email="junior@x.dev", password="Password123!", company=company
    )
    senior = await create_user(
        db_session, email="senior@x.dev", password="Password123!", company=company
    )
    await create_employment(
        db_session, user=junior, company=company, title="Engineer", years_ago_start=1
    )
    await create_employment(
        db_session, user=senior, company=company, title="Engineer", years_ago_start=5
    )
...
@pytest.mark.asyncio
async def test_invalid_employment_does_not_break_valid_tenant_results(
    db_session: AsyncSession,
) -> None:
    company = await create_company(db_session, "Invalid interval")
    invalid = await create_user(
        db_session, email="invalid@x.dev", password="Password123!", company=company
    )
    valid = await create_user(
        db_session, email="valid@x.dev", password="Password123!", company=company
    )
    ...
    await create_employment(
        db_session, user=valid, company=company, title="Engineer", years_ago_start=2
    )
