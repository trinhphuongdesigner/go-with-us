...
DEFAULT_GRANTS = {
    Role.BOD: [AdminPermission.COMPANY_READ, AdminPermission.EMPLOYEE_READ, AdminPermission.PASSPORT_APPROVE],
    Role.HR: [AdminPermission.COMPANY_READ, AdminPermission.EMPLOYEE_READ, AdminPermission.EMPLOYEE_WRITE, AdminPermission.ASSESSMENT_REVIEW],
}
...
async def company_read(db, row: Company) -> CompanyRead:
    ...
    return CompanyRead(id=row.id, name=row.name, status=row.status, version=row.version,
...
async def create_company(payload: CompanyCreate, db: DbSession, actor: CurrentUser):
    ...
    try:
        ...
        db.add(User(email=str(payload.admin_email).casefold(), name=payload.admin_name.strip(),
                    role=Role.COMPANY_ADMIN, company_id=company.id,
        ...
        for role, grants in DEFAULT_GRANTS.items():
            db.add(CompanyRoleDefinition(company_id=company.id, role=role.value, permissions=[item.value for item in grants]))
        await db.commit()
...
async def patch_company(company_id: uuid.UUID, payload: CompanyPatch, db: DbSession, actor: CurrentUser):
...
async def role_grants(db, company_id, role: Role) -> list[str]:
    ...
    definition = await db.scalar(select(CompanyRoleDefinition).where(
        CompanyRoleDefinition.company_id == company_id, CompanyRoleDefinition.role == role.value))
    return list(definition.permissions) if definition else [item.value for item in DEFAULT_GRANTS.get(role, [])]


@router.get("/users", response_model=list[AccountRead])
async def accounts(db: DbSession, actor: CurrentUser, company_id: uuid.UUID | None = Query(None, alias="companyId")):
    ...
    if actor.role == Role.SUPER_ADMIN:
        ...
        query = query.where(User.company_id == company_scope(actor, company_id), User.role.in_(get_manageable_roles(actor.role)))
...
async def create_account(payload: AccountCreate, db: DbSession, actor: CurrentUser):
    ...
    company_id = None if payload.role == Role.SUPER_ADMIN else company_scope(actor, payload.company_id)
    if company_id:
    ...
    row = User(email=str(payload.email).casefold(), name=payload.name.strip(), job_title=payload.job_title,
               role=payload.role, company_id=company_id, hashed_password=hash_password(payload.password.get_secret_value()),
               admin_permissions=await role_grants(db, company_id, payload.role))
    try:
        db.add(row)
        await db.flush()
        if is_employee_role(row.role):
            db.add(Employment(user_id=row.id, company_id=company_id, title=payload.job_title or "Nhân viên",
                              start_date=datetime.now(UTC), status=EmploymentStatus.ACTIVE))
        await db.commit()
    except IntegrityError:
        await db.rollback()
...
async def patch_account(user_id: uuid.UUID, payload: AccountPatch, db: DbSession, actor: CurrentUser):
    require(actor, Permission.PEOPLE_WRITE)
    ...
    if row.id == actor.id or not can_manage_role(actor.role, row.role) or not can_manage_role(actor.role, payload.role):
    ...
    row.email, row.name, row.job_title = str(payload.email).casefold(), payload.name.strip(), payload.job_title
...
class RoleGrantsPatch(ApiModel):
    ...
    permissions: list[AdminPermission] = Field(max_length=6)


@router.get("/roles", response_model=list[RoleGrantsRead])
async def list_roles(db: DbSession, actor: CurrentUser, company_id: uuid.UUID | None = Query(None, alias="companyId")):
    require(actor, Permission.COMPANY_READ)
    scope = company_scope(actor, company_id)
    rows = {row.role: row for row in (await db.scalars(select(CompanyRoleDefinition).where(CompanyRoleDefinition.company_id == scope))).all()}
    return [RoleGrantsRead(role=role, permissions=rows[role].permissions if role in rows else DEFAULT_GRANTS[role], version=rows[role].version if role in rows else 0) for role in DEFAULT_GRANTS]


@router.patch("/roles/{role}", response_model=RoleGrantsRead)
async def patch_role(role: Role, payload: RoleGrantsPatch, db: DbSession, actor: CurrentUser, company_id: uuid.UUID | None = Query(None, alias="companyId")):
    require(actor, Permission.COMPANY_MANAGE)
    if actor.role not in {Role.SUPER_ADMIN, Role.COMPANY_ADMIN} or role not in DEFAULT_GRANTS:
        raise HTTPException(403, "Chỉ quản trị viên được cấp quyền HR/BOD")
    scope = company_scope(actor, company_id)
    ...
    if company is None:
        raise HTTPException(404, "Không tìm thấy công ty")
    row = await db.scalar(select(CompanyRoleDefinition).where(CompanyRoleDefinition.company_id == scope, CompanyRoleDefinition.role == role.value))
    ...
    if row is None:
        row = CompanyRoleDefinition(company_id=scope, role=role.value, permissions=grants, version=1)
        db.add(row)
    else:
        row.permissions, row.version = grants, row.version + 1
    await db.execute(update(User).where(User.company_id == scope, User.role == role).values(admin_permissions=grants, version=User.version + 1))
    await db.commit()
