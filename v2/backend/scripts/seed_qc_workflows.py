...
from app.talent_workflows.schemas import Group, TemplateInput, TemplateRead
        ...
        db.add(
            PersonalDetails(
                ...
                details={
                    ...
                    "gender": "QC synthetic",
                    "idNumber": "QC-NOT-A-REAL-ID",
                    "emergencyContactName": "QC synthetic contact",
    ...
    if await db.get(ProfileActivityLog, activity_id) is None:
        db.add(
            ProfileActivityLog(
                ...
                title="QC: Chia sẻ kiến thức cùng đội ngũ",
                description="Hoạt động tổng hợp phục vụ kiểm tra giao diện và quyền; không phải thành tích thật.",
    ...
    for state in ("PENDING", "APPROVED"):
        ...
        if award is None:
            award = Award(
                ...
                user_id=user.id,
                company_id=user.company_id,
                name=f"QC: Ghi nhận đóng góp ({state})",
                type=AwardType.WORK,
                issuer="QC synthetic organization",
                description="Dữ liệu giả lập để kiểm tra quy trình xác nhận, không phải danh hiệu thật.",
                awarded_at=FIXTURE_DATE,
                self_reported=True,
        ...
        existing = await db.scalar(
            select(CompetencyRequest.id).where(
                CompetencyRequest.sender_id == user.id,
                CompetencyRequest.client_request_id == client_id,
            )
        )
        if existing:
            continue
        payload = RequestCreate(
            ...
            message="QC: Nhờ HR xác nhận đóng góp tổng hợp.",
        )
        snapshot = {
            "name": award.name,
            "issuer": award.issuer,
        ...
        db.add(
            CompetencyRequest(
                ...
                status=state,
                points_awarded=20 if state == "APPROVED" else 0,
...
async def add_roadmap_goals(db: AsyncSession, user: User) -> None:
    ...
    for roadmap in roadmaps:
        ...
        if await db.get(CareerGoal, identifier) is not None or await db.scalar(
            select(CareerGoal.id).where(CareerGoal.roadmap_id == roadmap.id)
        ):
            continue
...
async def seed_company_workflows(db: AsyncSession, company: Company, personas: list[User]) -> None:
    ...
    workforce = [
        ...
        if user.company_id == company.id and user.role in {Role.EMPLOYEE, Role.HR, Role.BOD}
    ]
    ...
    approvers = [
        ...
        and user.role in {Role.COMPANY_ADMIN, Role.BOD}
        and user.is_active
    ]
    ...
    if template is None:
        payload = TemplateInput(
            name="QC: Khung đánh giá phát triển",
            description="Mẫu tổng hợp phục vụ QC các vai trò, không phải thang đánh giá chính thức.",
            groups=[
                Group.model_validate(
                    {
                        "id": str(fixture_id(f"group:{company.id}:contribution")),
                        "name": "Đóng góp",
                        "weight": 60,
                        "scoreDimension": "CONTRIBUTION",
                        "questions": [
                            {
                                "id": str(fixture_id(f"question:{company.id}:delivery")),
                                "text": "Hoàn thành mục tiêu",
                                "guidance": "Dùng minh chứng đã thống nhất trong kỳ.",
                                "weight": 60,
                                "maxScore": 10,
                            },
                            {
                                "id": str(fixture_id(f"question:{company.id}:quality")),
                ...
                Group.model_validate(
                    {
                        "id": str(fixture_id(f"group:{company.id}:attitude")),
                        "name": "Thái độ",
                        "weight": 40,
                        "scoreDimension": "ATTITUDE",
                        "questions": [
                            {
                                "id": str(fixture_id(f"question:{company.id}:collaboration")),
                                "text": "Hợp tác cùng đội ngũ",
                                "weight": 50,
                                "maxScore": 10,
                            },
                            {
                                "id": str(fixture_id(f"question:{company.id}:learning")),
                                "text": "Chủ động học hỏi",
                                "weight": 50,
                                "maxScore": 10,
                            },
                        ],
        ...
        template = AssessmentTemplate(
            ...
            family_id=fixture_id(f"template-family:{company.id}"),
            name=payload.name,
            description=payload.description,
            groups=[group.model_dump(by_alias=True) for group in payload.groups],
            status="ACTIVE",
        )
        db.add(template)
        await db.flush()
    ...
    if cycle is None:
        ...
        cycle = await db.scalar(
            select(AssessmentCycle).where(
                AssessmentCycle.company_id == company.id, AssessmentCycle.period == "2026-09"
            )
        )
    if cycle is None:
        cycle = AssessmentCycle(
            ...
            name="QC: Chu kỳ phát triển tháng 09/2026",
            period="2026-09",
    ...
    for user in workforce:
        employment = await db.scalar(
            select(Employment)
            .where(Employment.user_id == user.id, Employment.company_id == company.id)
        ...
        answers = [
            {
                "questionId": q["id"],
                "score": max(1, min(q["maxScore"], round(q["maxScore"] * 0.8))),
                "comment": "QC: Minh chứng tổng hợp, không mô tả nhân sự thật.",
            }
        ...
        for state in ("DRAFT", "SUBMITTED", "APPROVED"):
            ...
            if await db.get(Assessment, identifier) is not None or (
                state == "APPROVED" and approver is None
            ):
                continue
            scores = (
                score(snapshot, answers, True)
                if state != "DRAFT"
                else {"total_score": None, "contribution_score": None, "attitude_score": None}
            )
            db.add(
                Assessment(
                    id=identifier,
                    ...
                    answers=answers[:1] if state == "DRAFT" else answers,
                    mood="Tốt",
                    highlights="QC: Đã hoàn thành buổi chia sẻ kiến thức tổng hợp.",
        ...
        for state in ("DRAFT", "APPROVED"):
            ...
            if await db.get(CareerSummary, identifier) is not None or (
                state == "APPROVED" and approver is None
            ):
                continue
            ...
            public_snapshot: dict[str, object] = {
                ...
                "content": content,
                "strengths": ["QC: Hợp tác", "QC: Chủ động học hỏi"],
                "growthAreas": ["QC: Tiếp tục thực hành"],
                "evaluation": "",
                "dimensionScores": {},
                "assessments": [],
                "skills": [],
                "approvedAt": FIXTURE_TIME.isoformat(),
            }
            db.add(
                CareerSummary(
                    id=identifier,
                    ...
                    status=state,
                    content=content,
                    strengths=public_snapshot["strengths"],
                    growth_areas=public_snapshot["growthAreas"],
                    approved_at=FIXTURE_TIME if state == "APPROVED" else None,
                    approved_by_id=approver.id if state == "APPROVED" and approver else None,
    ...
    if await db.get(JobRequirement, requirement_id) is None:
        db.add(
            JobRequirement(
                ...
                company_id=company.id,
                created_by_id=creator.id,
                title="QC: Phát triển nền tảng nội bộ",
                description="Nhu cầu tổng hợp để đối chiếu kỹ năng đội ngũ; không phải tin tuyển dụng thật.",
                required_skills=["Python", "React", "Communication"],
                status="open",
            )
        )
